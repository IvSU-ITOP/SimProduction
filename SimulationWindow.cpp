...
MainWindow::MainWindow ()
{
  ...
  QMenu *pMenuBase = new QMenu ("&Моделирование");
  pMenuBase->addAction ("&Ввести основные параметры...", this, SLOT (slotOpenDialog ()), QKeySequence ("CTRL+D"));
  m_pModeling = pMenuBase->addAction ("&Начать моделирование ", this, SLOT (slotOpenSumulation ()), QKeySequence ("CTRL+S"));
  menuBar ()->addMenu (pMenuBase);
  pMenuBase->addAction ("&Конец работы ", qApp, SLOT (quit ()), QKeySequence ("CTRL+Q"));
  menuBar ()->addMenu (pMenuBase);
  QMenu *pMenuTable = new QMenu ("&Таблицы");
  m_pTableNine = pMenuTable->addAction ("Отобразить Таблицу 9...", this, SLOT (slotShowTableNine ()), QKeySequence ("CTRL+N"));
  m_pTableTen = pMenuTable->addAction ("Отобразить Таблицу 10...", this, SLOT (slotShowTableTen ()), QKeySequence ("CTRL+T"));
  m_pMachineChartOne = pMenuTable->addAction ("Отобразить Машинограмму №1...", this, SLOT (slotShowMachineChartOne ()), QKeySequence ("CTRL+O"));
  m_pMachineChartTwo = pMenuTable->addAction ("Отобразить Машинограмму №2...", this, SLOT (slotShowMachineChartTwo ()), QKeySequence ("CTRL+M"));
  m_pMachineChartFour = pMenuTable->addAction ("Отобразить Машинограмму №4...", this, SLOT (slotShowMachineChartFour ()), QKeySequence ("CTRL+F"));
...
void MainWindow::slotShowTableTen ()
{
  ...
  QStringList lst;
  lst << "Название детали" << "Суммарное время \n (штучное)" << "Суммарное время \n подготовительно-\nзаключительное"
    << "Размер партии \n предварительный" << "Размер партии \nпринятый " << "Время выполнения\n технологических\n операций "
    << "Суммарное время\n межоперационного \n пролеживания" << "Длительность \nпроизводственного\n цикла\n на проектируемом\n участке (час.)"
    << "Длительность \n  производственного  \n цикла \nна проектируемом\n участке (сут.)" << "Время обработки \nв других \nцехах (час)"
    << "Время обработки\n в других \nцехах (сут.)" << "Обшая длительность\n производственного\n цикла (сут.)";
...
void MainWindow::slotShowMachineChartOne()
  ...
  QTableWidgetItem *pi = 0;
  QStringList lst;
  lst << "Номер изделия" << "План" <<  "Факт"
    << "Отклонение" << "Некомплектованные\n детали" << "Некомплектованные\n детали"
    << "Некомплектованные\n детали " << "Потребность до  \n планового объема"
    << "Потребность до  \n планового объема"  << "Потребность до  \n планового объема";
  tbl->setHorizontalHeaderLabels( lst );
  tbl->horizontalHeader()->setSectionResizeMode( QHeaderView::Stretch );
...
void MainWindow::slotShowMachineChartTwo()
  ...
  QStringList Lst;
  Lst << "Название детали" <<  "НЗП на начало"  <<  "Готовые детали \n на начало"
    <<  "Запуск" << "НЗП на конец"  <<  "Готовые детали \n на конец"
    << "Деталей в изделиях" << "Баланс \n Н - К";
...
void MainWindow::slotShowMachineChartFour ()
{
  ...
  QTableWidgetItem *pItem = 0;
  QStringList lst;
  lst << "Наименование группы\n оборудования" << "Режимный фонд\n времени" << "Общий фонд\n времени"
    << "Время ремонта\n оборудования" << "Простои по причине\n отстутствия рабочих" << "Время простоя\n оборудования"
    << "Время на переналадку\n оборудования" << "Время работы" << "Общее время\n работы";
  tbl->setHorizontalHeaderLabels (lst);
  tbl->horizontalHeader ()->setSectionResizeMode (QHeaderView::Stretch);
  int i = 0;
...
m_pTimeSimulation( new QSpinBox ), m_pVariant( new QSpinBox )
  ...
  pHLayout->addWidget( new QLabel( "Введите номер Вашего варианта" ) );
  ...
  pHLayout->addWidget( new QLabel( "Введите длину смены " ) );
  ...
  pVLayout->addLayout( pHLayout );
  pHLayout = new QHBoxLayout;
  pHLayout->addWidget( new QLabel( "Введите количество смен " ) );
  ...
  pVLayout->addLayout( pHLayout );
  pHLayout = new QHBoxLayout;
  pHLayout->addWidget( new QLabel( "Введите время начала рабочего дня " ) );
  ...
  pVLayout->addLayout( pHLayout );
  pHLayout = new QHBoxLayout;
  pHLayout->addWidget( new QLabel( "Введите продолжительность симуляции" ) );
  ...
  QPushButton *pSelect = new QPushButton( "Сохранить" );
